...
5. **`db reset` exige el stack completo:** falla si se arrancó con `-x`. Y **Docker Desktop tiene que estar
   arrancado** para `supabase db pull` y `supabase start`. ⚠ **Y con Docker parado, `npx supabase start`
   imprime su error y sale con código 0** —medido el 2026-08-19: `LegacyDockerLifecycleInspectError` en la
   salida, `exit 0` en el código—. **Que el stack esté arriba se comprueba por el efecto**, no por el
   código de salida: `docker ps` lista los contenedores, o `netstat -ano` encuentra el 54322. Es la tercera
   regla cara del global cobrándose en la herramienta con la que empieza cada sesión.
6. ⚠ **`products.description` no es una descripción: son tres campos en uno**, `Lab: <salón> | <especificación> | Obs: <estado>`, en **34 de 34**. El tercero es información de estado del equipo, o sea **lo mismo que la migración 25 hizo privado del personal** *(D-69)*, publicado a cualquier alumno. Se desempaqueta en la F3-T1 *(D-82)*. ~~**Hasta entonces, nada que muestre esa columna es seguro de ampliar.**~~ ~~⚠ **Precisado el 2026-08-18: el desempaquetado existe pero NO está en producción.**~~ ✅ **CERRADO el 2026-08-19: la migración 28 está aplicada en producción** (PR #38, `db push`, las 29 migraciones con `local` y `remote` idénticos). **Verificado por el efecto y con controles:** 0 descripciones empaquetadas, 0 publicando observación, las 2 vacías previstas, sobre 34 productos y 32 con texto. **La columna ya no publica el estado del equipo.**

> ⚠ **Y dejó una trampa nueva que vale para cualquier migración que MUEVA un dato: la 28 creó 41 notas duplicadas.** No insertó de más —metió 50 notas en 50 unidades, una por unidad, que es lo que D-82 pedía—: **`inventory_unit_notes` ya contenía esas mismas observaciones desde el 2026-02-24**, y nadie lo miró. El diseño midió el **origen** con cuidado —34 descripciones, tres formas, 92 unidades— y **no midió el destino**. **Antes de mover un dato a una tabla, se cuenta también lo que esa tabla ya tiene.** Lo delató que `notas_migradas` diera **108** contra **92 unidades**: un total que supera al universo posible no es un total, es una definición que mide otra cosa. **Arreglado el mismo día con la migración 30** *(D-83)*: de 109 notas a **68**, cero duplicados, y **las 53 unidades con nota intactas** — se borraron copias, no notas.
