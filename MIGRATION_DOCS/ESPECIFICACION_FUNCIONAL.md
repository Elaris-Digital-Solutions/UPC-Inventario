...
| M-12 | **Cancelación con antelación mínima** | Hoy se puede cancelar un minuto antes sin consecuencia | ✅ **Hecha entera el 2026-08-15, en dos mitades y dos migraciones.** **D-38** cerró la primera —no se cancela una reserva que **ya empezó**, migración 23— y **D-70 y D-71** la segunda: `app_settings` gana `min_cancel_minutes` con `default 60`, editable desde `/admin/ajustes`, y `cancel_reservation` compara contra `now()` más el margen —**migración 26**—. **El margen exime al personal** *(D-71)*, igual que D-38 y por el mismo motivo: el personal ya cancela por `UPDATE` directo y ese camino no mira la hora. **Un margen de 0 apaga la regla y devuelve el comportamiento de D-38 exacto**, que es por qué el `check` admite el cero: deja desactivarla sin otra migración. **La comprobación va en dos ramas con dos mensajes** y no en una comparación ampliada, porque `31_cancel_before_start.sql` **afirma el texto** del mensaje de D-38 y ampliarla lo habría roto. ~~La T4 no la construye *(D-55: una sola migración esta tanda)*.~~ *(Cierto al escribirlo el 2026-08-13; la construyó la tanda 5 dos días después.)* ~~**En local; en producción sigue abierta hasta que se empuje la rama**~~ ⚠ **Corregido el 2026-08-16, unas horas después:** **también en producción**: `min_cancel_minutes` vale **60** con su `check (0..1440)`, y `cancel_reservation` trae **las dos ramas con sus dos mensajes**, el de D-38 primero |
...
⚠ **Corregido el 2026-08-15, al cerrar la tanda 5, y lo que caducó no es el número sino una palabra:** «se
decidieron tres» **sigue siendo cierto** —M-9, M-11 y M-12— y lo que dejó de serlo es **«media»**, porque la
tanda 5 cerró la otra mitad de M-12 con las migraciones 25 y 26 *(D-70, D-71)*. **Y la frase siguiente no se
toca:** M-10 continúa siendo la única que espera criterio, y sigue siendo **Q-4**.
