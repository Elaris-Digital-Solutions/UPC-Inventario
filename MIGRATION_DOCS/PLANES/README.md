...
| [`FASE_3_TANDA_5.md`](./FASE_3_TANDA_5.md) | PWA: `manifest` e iconos, y **la reverificación completa de la CSP** *(D-81)*. **Cero migraciones, cero SQL y cero dependencias nuevas** | ⚠ **PLANIFICADA el 2026-08-19, sin ejecutar** · **8 correcciones al diseño** · 6 tareas · 4 puntos a verificar · **D-88 y D-89** · ⚠ **la corrección principal QUITA alcance: instalar NO necesita service worker** —la guía de Next 16 instalada pide manifest y HTTPS— **y el botón de instalar propio lo desaconseja esa misma guía**, por no funcionar en Safari iOS · ⚠ **`primer_acceso_personal()` no puede condicionar el aviso al alumno**: lee de `staff_members` y exige `is_admin()`, o sea **cero filas siempre** · **la tarea de riesgo va primera, con las dos salidas escritas por delante**: si Chrome exigiera un service worker, **se para y decide Alejandro** · **Q-20 se mide dentro de su recorrido de CSP** *(D-89)*: de 4 superficies a 18, **y son 18 y no 17 porque la F3-T3 añadió un Dialog** |
