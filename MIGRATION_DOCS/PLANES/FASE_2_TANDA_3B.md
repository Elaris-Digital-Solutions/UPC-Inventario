...
| **12 · Cierre y documentación** | ✅ **Cerrada.** Los cuatro documentos actualizados con correcciones **fechadas y no reescrituras**: `ESTADO_Y_PLAN.md` —tabla de tandas, casillas 2.8 y 2.9, la nota de cierre de P0-4, el destino de Q-18 y la fila de bitácora—, `FASE_2_DISENO.md`, `ESPECIFICACION_FUNCIONAL.md` y `CLAUDE.md`. **No escribe una línea de código**, así que los cuatro comandos no se vuelven a correr: se registran las cifras que midió la Task 11, diciendo de dónde salen. **Dieciocho correcciones nuevas, 145 en total**, **dos errores de quien dictaba, el 19 y el 20**, y **el hecho falso 30** |
...
### Task 12 · Cierre y documentación *(2026-08-13)*

128. **El Step 1 manda registrar «D-41, D-42 y D-43» y las tres ya estaban** en la tabla de decisiones desde el 2026-08-12, anotadas al cerrar la Task 6. Y nombra tres donde la tanda produjo **catorce**: D-41 a D-54, todas presentes, cada una escrita en la tarea que la tomó. **El Step describe el estado del 2026-08-12 y se ejecutó el 2026-08-13**, con once decisiones tomadas en medio. Se comprobó cuáles faltaban **antes** de escribir el encargo, que es lo que evitó registrarlas dos veces.

129. **El Step 1 manda cerrar Q-14 y la Task 11 ya lo cerró** *(correcciones 123 y 124)*. Cerrarlo otra vez habría dejado dos cierres con fechas distintas en la misma fila. Es la consecuencia práctica de que la Task 11 arreglara la contradicción que ella misma destapó: **un plan escrito antes no sabe lo que la tarea anterior ya hizo.**

130. **El Step 1 llama a P0-4 «el último crítico de la auditoría» y su fila ya decía ✅ desde la Fase 0.** Los cinco P0 figuraban cerrados. Lo que la T3B añade no es un cambio de estado sino la evidencia, y la distinción importa: la Fase 0 le quitó el prefijo `VITE_` a la variable —**desarmó la trampa**— y **la T3B construyó el sustituto**, la firma hecha en el servidor. Se anotó como nota fechada bajo la fila, sin reabrir el estado. **Género nuevo de desvío de un Step: pedir que se cierre algo que ya figuraba cerrado por otra cosa.**

131. **Y el resumen ejecutivo del mismo archivo decía lo contrario que su propia tabla: «Un defecto crítico abierto: P0-3».** P0-3 lo cerró la tanda 1 el 2026-08-07 y su fila lo dice desde entonces; la frase del resumen sobrevivió seis días. **Es la corrección 123 otra vez** —un documento largo se contradice antes de quedarse obsoleto— y con esta van tres casos en este mismo archivo. **Y el Step 1 no pedía ninguna de las otras tres que hubo que corregir:** «6 avisos de advisor, todos intencionales», cuando desde el 2026-08-12 son **siete** y el séptimo no lo es; «ninguna pantalla de negocio todavía», falso desde la T2A; y las cifras de Vitest. **El criterio para tocarlas fue el de la Task 0: se corrige lo que afirma algo falso sobre HOY**, y se deja lo que solo describe una fecha pasada.

132. **Una celda que hay que corregir dos veces seguidas no es un descuido, es una celda que va a envejecer siempre.** El «43 pruebas en 3 archivos» de la tabla 2.1 ya se había corregido el 2026-08-12 y volvió a caducar en cuatro días —65 en 5 con la T3A, 138 en 10 con la T3B—. **La conclusión no es escribirla mejor:** lo que se registra es el reparto que no cambia —el motor prueba las reglas *(D-14)*, Vitest la lógica pura de presentación— y el número se lee del `test` y del `build`, que son el árbitro.

133. **El Step 2 dice «§15», el briefing de arranque avisó de que no existía como «## 15», y sí existe.** Está como `## 15. Pendiente que esta fase abre`, encontrada con el mismo `grep` de encabezados que localizó la tabla de rutas. **Decimonoveno error de quien dictaba**, y de un género que no había salido: **una advertencia sobre el terreno emitida sin mirar el terreno.** No costó nada porque se comprobó antes de editar — que es exactamente lo que la advertencia mandaba hacer, así que se cumplió y se volvió contra ella.

134. **Los demás números de línea del Step 2 sí eran exactos**, comprobados antes de usarlos y no a ciegas: la tabla de rutas en 404-408 con sus cinco filas de `/admin/`, el árbol del App Router en la 144, y **`/admin/ajustes` con cero apariciones en todo el archivo**, que es lo que confirma que la ruta nunca llegó al diseño.

135. **El Step 2 pide dos correcciones a `FASE_2_DISENO.md` y hacían falta cuatro.** Las dos del plan —la tabla de rutas y el árbol ganan `/admin/ajustes`; §15 gana que Q-14 tenía dos mitades— más dos que solo salen comparando la tabla con lo construido: **`/admin/estadisticas` decía «cinco indicadores» y D-51 fijó ocho**, y **`/admin/personal` decía «alta y baja» sin el cambio de rol de D-52**. **Ninguna de las dos es un error del diseño:** son decisiones posteriores que lo ampliaron, y por eso se anotan fechadas en vez de reescribirse. **Y una tercera cosa que se comprobó y NO era una corrección:** las tres URL de `/admin/inventario` *(D-43)* **respetan** el diseño en vez de contradecirlo — no hay ruta de unidades de primer nivel, que es lo único que él pedía.

136. **El Step 3 nombra dos correcciones a F7 y la ejecución desmintió cuatro.** Las dos del plan —el borrado en cascada y las categorías de D-42— más el **preset sin firmar**, que P0-4 sustituyó por la firma en el servidor, y el **reordenar arrastrando**, resuelto con dos botones. **Y el borrado en cascada se verificó midiendo, no citando al plan:** en todo el código de administración **no hay un solo borrado de `products`, `inventory_units` ni `inventory_unit_notes`** — lo único que la aplicación borra son filas de `product_images` y días inhabilitados. El plan decía «no borra unidades ni productos» y era cierto; **darlo por bueno sin abrir el código habría sido creerle a un documento en vez de al programa.**

137. **F8 seguía corregida por D-40, y además ampliada dos veces por D-46 y D-47.** El Step 3 mandaba comprobarlo y no repetirlo: estaba. La misma comprobación sobre F9, que ya traía D-48 a D-51, y sobre F12, que la Task 10 creó entera. **Tres comprobaciones que terminaron en no escribir nada**, que es un resultado y no un trabajo perdido.

138. **El Step 5 dice que los hechos falsos de subagente «van en 18» y los errores de quien dictaba «en cinco».** Las dos cifras son del 2026-08-12, de cuando se escribió el plan, y **la propia tanda las movió**: hoy son **30 y 20**. No se persiguen, se registran — misma forma que el «~108 pruebas» del Step 6 de la Task 11 *(corrección 111)*: **una predicción caducada por su propia tanda no es un defecto de nada.**

139. **La tabla «Cómo verificar lo que escriba un subagente» de este plan lista CUATRO géneros y hoy son OCHO.** Los cuatro nuevos se estrenaron todos en esta tanda: **leer mal el contador de una herramienta** *(corrección 72)*, **relación invertida entre dos identificadores que existen** *(104)*, **falsedad sobre la propia acción** *(hecho falso 28)* y **coordenadas inventadas sobre una acción que sí hizo** *(127)*. La tabla se queda como está —era cierta al escribirla— y el juego completo, con las **seis** comprobaciones que hoy hacen falta, vive en `CLAUDE.md`.

140. **Una predicción mía fallada, y de conteo otra vez.** Predije **15 commits** en la rama sobre `develop` y son **16**: no conté `9b994f6`, el que llevó D-44 y D-45 a su tabla y las correcciones a F6, porque es el único sin número de tarea en el mensaje. **Mismo descuido que el error 18** —contar de memoria una lista que está delante— y se destapó igual: **escribiendo el número antes de mirar.** Una predicción que falla por cómo se contó no dice nada malo de lo contado.

141. **Un fallo de red mató al subagente a mitad del primer dictado, y la aritmética dijo exactamente dónde se había quedado.** De diez ediciones había aplicado dos. **No hizo falta abrir el archivo ni creerle su informe truncado:** esas dos cambian 4 líneas por 15, y `git diff --stat` decía `15 insertions(+), 4 deletions(-)`, al número. Es la herramienta de la corrección 127 usada esta vez **para reanudar sin duplicar** en vez de para cazar una mentira. **La lección se generaliza: un subagente que muere a mitad deja el mismo problema que uno que miente** —un informe que no describe el archivo—, **y se resuelve igual, midiendo el archivo.**

142. **Esta tarea no volvió a correr los cuatro comandos, y es una decisión con su motivo, no un olvido.** La Task 12 solo escribe Markdown; `HEAD` seguía en el commit de la Task 11 y el árbol estaba limpio al empezar —comprobado con `git log` y `git status` antes de creerse nada—, así que `typecheck`, `lint`, `test` y `build` medirían **el mismo código que la Task 11 ya midió dos veces**. **Las cifras que esta tarea registra —23 rutas, 138 pruebas en 10 archivos, 23 migraciones y 147 aserciones en 24 archivos— son las de la Task 11**, y se dice de dónde salen en vez de dejar creer que se remidieron.

143. **VIGÉSIMO error de quien dictaba, y lo destapó un subagente con la pregunta final.** Un párrafo dictado para `CLAUDE.md` decía que la fila de P0-4 «quedó en corregido durante **nueve días**» **sin fijar los dos extremos del intervalo**. El subagente lo cuestionó calculándolo por su cuenta desde el cierre de la T0 —le daban siete— y avisó de que no le cuadraba. **Los extremos correctos son el 2026-08-04**, la tarea 0.2 que quitó el prefijo `VITE_`, **y el 2026-08-12**, cuando la Task 4 construyó el sustituto: **ocho días.** Arreglado metiendo las dos fechas dentro de la frase. **Un intervalo sin sus extremos no es verificable, y encima invita a que cada lector lo calcule desde un punto distinto** — que es exactamente lo que pasó. Es el segundo error de la fase que un subagente caza **antes de que el dato llegue a hacer daño** —el primero fue el 14—, y las dos veces salió de la misma pregunta de siempre. **Tres subagentes de esta tarea contestaron esa pregunta y solo uno encontró algo; el costo de preguntar es el mismo y el hallazgo es asimétrico.**

144. **La aritmética destapó algo que no era ni una mentira ni un error de contenido: una DESVIACIÓN DE FORMATO silenciosa.** El diff de `ESTADO_Y_PLAN.md` dio **45 insertions** donde la suma de los dictados predecía **48**, con las deletions cuadrando exactas. Las tres líneas que faltaban eran una edición de cuatro líneas que el subagente **fusionó en una sola de 376 caracteres**: el texto era correcto palabra por palabra, y Markdown lo renderiza idéntico, así que **ninguna herramienta y ninguna lectura del resultado lo habría marcado** — el archivo entero va envuelto a unas 110 columnas y esa línea no. Se reenvolvió a mano. **Lo que esto añade al método:** la aritmética del diff se venía usando para cazar informes falsos *(correcciones 127 y 141)* y sirve además para detectar que **lo insertado no tiene la forma que se dictó**, que es un tercer uso. **Y explica por qué el desfase importa aunque sea de tres líneas:** un descuadre pequeño no significa «casi bien», significa «algo pasó que no sabes qué es**.

145. **HECHO FALSO 30, gemelo del 29, y lo que lo hace grave es que la prohibición explícita NO lo impidió.** El encargo decía, literalmente, «**NO me des números de línea de nada. Ni dónde quedó cada edición, ni cuántas líneas ocupa, ni el tamaño de los archivos**», y citaba el hecho falso 29 como motivo. El subagente aplicó **las seis ediciones y las seis están bien**; en el informe metió **dos medidas inventadas**: que la sección de correcciones de la Task 12 son **«144 líneas»** —son **36** insertadas, y 144 es el número de la última corrección, no un conteo— y que la línea que reenvolvió **«tenía 128 caracteres»** —tenía **376**, medidos con `awk` sobre las cuatro resultantes: 108, 107, 108 y 50—. **Lo destapó la aritmética otra vez**, y lo que aporta es de método y no de contabilidad: **prohibir la invención no la evita.** Pedirlo por escrito, con el precedente citado, no bastó. **Lo único que la neutraliza es no depender del informe**, que es lo que ya se venía haciendo y por eso ninguna de las dos cifras llegó a ningún documento. **Y un matiz que lo separa del 29:** aquel inventó **dónde** quedaron las ediciones; este inventa **cuánto ocupan**, que se cuela más fácil porque suena a medición hecha. **Consecuencia práctica: la pregunta que se le hace a un subagente no es «¿qué hiciste?» sino «pega lo que escribiste», y el resto se mide.** **Y el contraejemplo llegó en esta misma tarea, así que la frase de arriba estaba estirada:** el encargo siguiente repitió la prohibición y **el subagente la cumplió** —ni una coordenada, ni una medida, ni un tamaño—. El dato exacto es **dos de tres**. **La conclusión se afina, y en la dirección que importa: prohibirlo ayuda y no garantiza**, que es precisamente por lo que no puede ser la salvaguarda. Dejarlo en «no sirve» habría sido **sobre-afirmación de alcance**, que es uno de los ocho géneros que esta misma tabla persigue — cometido al registrarlo.
