...
- **Cada `fetch` saliente deja en Sentry un breadcrumb con su query, y a PostgREST los filtros van en la query** *(medido el 2026-09-23, `@sentry/node` 10.70)*. La query va aparte, en `data["http.query"]`, y `data.url` llega sin ella. `.eq('email', …)` se convierte en `email=eq.<correo>` en esa miga. Lo quita `limpiarEvento` *(H-21)*. **El cuerpo de la petición entrante no se capturó** en un servidor `http` de Node; en el runtime de Netlify está sin medir. ⚠ **Lo que se ve en `beforeSend` no es lo que se envía:** el evento trae ahí `sdkProcessingMetadata.normalizedRequest`, con la URL, la cookie y el `authorization`, y **el SDK lo quita al armar el envelope**. **El instrumento que decide es el envelope:** `Sentry.init` con `dsn: http://clave@127.0.0.1:<puerto>/1` apuntando a un `http.createServer` local que guarda lo que recibe, y se buscan los marcadores en ese texto. La sonda completa quedó descrita en la bitácora del 2026-09-23.
