...
- **«Base de datos» o «E2E» caen al levantar Supabase con `toomanyrequests: … allowed: 44000/minute`, y no es el código** *(medido el 2026-09-23 en el PR #70)*. Es `ghcr.io` limitando las descargas **anónimas** de imágenes: `supabase/setup-cli` exporta `SUPABASE_INTERNAL_IMAGE_REGISTRY=ghcr.io` para cualquier CLI desde la 1.28.0, y el CI no hace `docker login`. **Se reconoce** porque el log no tiene ninguna línea `Applying migration`. **Se descarta el PR** relanzando la última corrida verde de `develop`: si falla igual, no es el PR. **Se resolvió solo:** el tercer intento, unas 6 h después, bajó del mismo registro y pasó. Cuánto dura el bloqueo **no está medido**, y la alternativa `public.ecr.aws` tampoco *(`ESTADO_Y_PLAN.md`, bitácora del 2026-09-23)*.
- **El Build del CI cae con `Module not found: Can't resolve '@vercel/turbopack-next/internal/font/google/font'`, y no es el código** *(medido el 2026-09-25, run `36156124258`)*. Es `next/font/google` que no pudo bajar Playfair Display de Google durante el build; la traza apunta a `app/layout.tsx`. **Se reconoce** porque el mismo árbol pasó en el PR: el merge `6dfb15c` y la cabeza del PR `6b5c9e3` tenían `git diff` vacío. **Relanzado con `gh run rerun <id> --failed`, pasó al intento 2.**
- **El `execute_sql` del MCP de Supabase no puede llamar a una RPC que sólo ejecuta `authenticated`** *(medido el 2026-09-23 contra producción)*. Entra como `supabase_read_only_user`, en una transacción de solo lectura y sin poder actuar como `authenticated`, así que `available_slots(...)` da **`42501: permission denied for function`**. **Ese error habla del instrumento, no de la función.** Para comprobar que una RPC de producción es la que se probó en local: `md5(prosrc)` de `pg_proc` en los dos lados, con una función que no cambió como control. Los permisos se leen con `has_function_privilege` y `has_table_privilege`, que sí funcionan con ese rol.
- **Qué manda Sentry con un error de PostgREST depende de si es un objeto plano o un `Error`** *(medido el 2026-09-23, `@sentry/node` 10.70 y `postgrest-js` 2.112)*. El `error` de `{ data, error }` es un **objeto plano** —`JSON.parse` del cuerpo; sólo `throwOnError()` lanza un `PostgrestError`, que sí extiende `Error`—, y `captureException` lo copia **entero** a `extra.__serialized__`, `details` incluido: *«Failing row contains (…)»*, la fila completa. **Un `Error` con las mismas propiedades no las manda.** Lo cierra el `delete evento.extra` de `limpiarEvento` *(H-21)*. **Se mide sin enviar nada:** `Sentry.init` con un DSN inventado y un `beforeSend` que imprime el evento y devuelve `null`. ⚠ **Y la sonda miente si el texto buscado está escrito en su propio fuente:** el SDK adjunta las líneas de código alrededor de cada frame en `stacktrace.frames[].pre_context`, así que un `'Failing row'` literal en el script aparece en el evento **aunque la corrección funcione**. Se arma en ejecución —`['Failing', 'row'].join(' ')`— y se nombra la ruta donde aparece, no sólo si aparece.
- **Cada `fetch` saliente deja en Sentry un breadcrumb con su query, y a PostgREST los filtros van en la query** *(medido el 2026-09-23, `@sentry/node` 10.70)*. La query va aparte, en `data["http.query"]`, y `data.url` llega sin ella. `.eq('email', …)` se convierte en `email=eq.<correo>` en esa miga. Lo quita `limpiarEvento` *(H-21)*. ~~**El cuerpo de la petición entrante no se capturó** en un servidor `http` de Node; en el runtime de Netlify está sin medir.~~ ⚠ **Corregido el mismo día, midiendo con `next start`: SÍ se captura.** En una Server Action, `request.data` lleva los argumentos serializados del formulario. **El servidor `http` simple no reproduce lo que hace Next**, así que una sonda así no sirve para medirlo. Lo quita `limpiarEvento` *(H-21)*. ⚠ **Lo que se ve en `beforeSend` no es lo que se envía:** el evento trae ahí `sdkProcessingMetadata.normalizedRequest`, con la URL, la cookie y el `authorization`, y **el SDK lo quita al armar el envelope**. **El instrumento que decide es el envelope:** `Sentry.init` con `dsn: http://clave@127.0.0.1:<puerto>/1` apuntando a un `http.createServer` local que guarda lo que recibe, y se buscan los marcadores en ese texto. La sonda completa quedó descrita en la bitácora del 2026-09-23.
- **Un `console.error` no crea un evento en Sentry** *(medido el 2026-09-23 sobre el envelope)*: llega **sólo como miga** del evento siguiente, y si no hay evento siguiente, no llega. Un fallo que se registra con `console.error` y sigue queda fuera de Sentry, y en Netlify el stdout se rota. Por eso todo fallo inesperado pasa por `reportar()` *(H-22)*.
- **PostgREST llamado directamente con la clave pública revela estructura, no datos** *(medido el 2026-09-23 contra producción)*. `GET /rest/v1/` → **401** *«Secret API key required»*: el OpenAPI está cerrado. Pero una tabla inexistente parecida a una real → `404 PGRST205` con *«Perhaps you meant the table 'public.alumnos'»*; una real sin `GRANT` para `anon` → `401 42501` con el `GRANT` sugerido; una columna inexistente → `400 42703`; una función inexistente → `404 PGRST202`. **Aquí no agrega exposición**, porque el esquema entero está en las migraciones del repositorio público.
