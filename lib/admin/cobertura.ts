...
//
// EL FALLO QUE ESTO HACE VISIBLE: con D-74, una sede sin turnos cargados no
...
// pregunta por que.
//
// PURA Y SIN RED, con IMPORTS RELATIVOS porque bajo Vitest el alias `@/` no
// resuelve (COMPORTAMIENTO_MEDIDO.md §5). Por eso esto no vive en semana.ts, que
// si importa con alias.

// Las TRES formas que avisan, mas la cuarta que es "no pasa nada". Se nombran las
// cuatro: un dia CUBIERTO tiene que poder distinguirse de uno que no se calculo.
...
export type DiaDeCobertura = {
  ...
  campusId: string;
  forma: FormaDelDia;
  // Cero salvo en 'parcial': en 'sin-operador' seria el dia entero, y decirlo
  // alli seria repetir con un numero lo que la forma ya dice con una palabra.
...
// Los segundos se ignoran a proposito: las dos tablas son `time` sin fraccion y
// ninguna pantalla los ofrece, asi que un segundo suelto seria un dato que nadie
// pudo escribir.
...
// El dia de la semana de una fecha CIVIL, con 0 = domingo.
//
// Se construye en UTC A PROPOSITO: `new Date('2026-08-21')` tambien parsea como
// UTC, pero `getDay()` lo devolveria en la zona del navegador, y en Lima eso
// corre la fecha un dia hacia atras. Aqui no hay ningun instante que convertir.
...
/**
 * La cobertura de cada sede en los proximos `dias` dias.
 *
 * `dias` sale de `app_settings.booking_window_days` (D-3) y NO de un 7 escrito a
 * mano: un panel que mirase siete dias mientras el alumno reserva a diez avisaria
 * tarde de los tres ultimos.
 */
